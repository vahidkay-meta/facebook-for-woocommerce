...
jQuery(document).ready(function ($) {
	...
	if ("edit-product" === pagenow) {
		...
		$("input#doaction, input#doaction2").on("click", function (e) {
			...
			if ("facebook_include" === chosenBulkAction) {
				...
				$.post(
					facebook_for_woocommerce_products_admin.ajax_url,
					{
						action: "facebook_for_woocommerce_set_product_sync_bulk_action_prompt",
						security:
							facebook_for_woocommerce_products_admin.set_product_sync_bulk_action_prompt_nonce,
	...
	if ("product" === pagenow) {
		...
		function toggleFacebookSellOnInstagramSetting(enabled, $container) {
			...
			$field
				.prop("checked", enabled ? checked : false)
				.prop("disabled", !enabled);
			...
			$container
				.find(
					"#product-not-ready-notice, #variable-product-not-ready-notice",
				)
				.hide();
		...
		function isSyncEnabledForVariableProduct() {
			...
			return !!$fields.map((i, element) =>
				$(element).val() !== "sync_disabled" ? element : null,
			).length;
		}
		...
		function isPriceDefinedForSimpleProduct() {
			return !!(
				$("#_regular_price").val() || $("#fb_product_price").val()
			);
		}
		...
		function isStockManagementEnabledForSimpleProduct() {
			return !!(
				$("#_manage_stock").prop("checked") && $("#_stock").val()
			);
		}
		...
		function shouldShowMissingGoogleProductCategoryAlert() {
			...
			let selectedCategories = $(
				".wc_facebook_commerce_fields .wc-facebook-google-product-category-select",
			).map((i, element) => {
		...
		function showMissingGoogleProductCategoryAlert(event) {
			...
			alert(
				facebook_for_woocommerce_products_admin.i18n
					.missing_google_product_category_message,
			);
		...
		function shouldShowProductRemovedFromSyncConfirmModal($syncModeSelect) {
			let syncValuesStatus = $syncModeSelect
				.map(function (index, selectElement) {
					let $syncMode = $(selectElement);
					let syncModeValue = $syncMode.val();
					let isProductPublished =
						!!facebook_for_woocommerce_products_admin.is_product_published;

					return (
						isProductPublished &&
						"sync_disabled" === syncModeValue &&
						syncModeValue !== $syncMode.attr("data-original-value")
					);
				})
				.toArray();
		...
		function getSyncTargetProductID($syncModeSelect) {
			...
			// variable product
			return $syncModeSelect
				.closest(".woocommerce_variation")
				.find("input[name^=variable_post_id]")
				.val();
		}
		...
		function showProductRemovedFromSyncConfirmModal($syncModeSelect) {
			...
			maybeRemoveFromSyncProductID =
				getSyncTargetProductID($syncModeSelect);

			new $.WCBackboneModal.View({
				target: "facebook-for-woocommerce-modal",
				string: {
					message:
						facebook_for_woocommerce_products_admin.product_removed_from_sync_confirm_modal_message,
					buttons:
						facebook_for_woocommerce_products_admin.product_removed_from_sync_confirm_modal_buttons,
				},
			});
		}
		...
		function populateRemoveFromSyncProductIDsField() {
			$(
				facebook_for_woocommerce_products_admin.product_removed_from_sync_field_id,
			).val(removeFromSyncProductIDs.join(","));
		}
		...
		function removeProductIDFromUnSyncList(productID) {
			removeFromSyncProductIDs = removeFromSyncProductIDs.filter(
				function (value) {
		...
		$(document.body)
			.on(
				"click",
				"button.button-product-removed-from-sync-delete",
				function () {
					if (maybeRemoveFromSyncProductID) {
						closeExistingModal();

						removeFromSyncProductIDs.push(
							maybeRemoveFromSyncProductID,
						);
			...
			.on(
				"click",
				"button.button-product-removed-from-sync-cancel",
				function () {
					closeExistingModal();

					if ($maybeRemoveFromSyncModeSelect) {
						revertSyncModeToOriginalValue(
							$maybeRemoveFromSyncModeSelect,
						);
		...
		$("#facebook_options #wc_facebook_commerce_enabled")
			.on("change", function () {
				...
				if (
					$(".product_attributes").find(".woocommerce_attribute")
						.length
				) {
		...
		const facebookSettingsPanel = simpleProductSyncModeSelect.closest(
		...
		simpleProductSyncModeSelect
			.on("change", function () {
				let syncEnabled =
					simpleProductSyncModeSelect.val() !== "sync_disabled";

				toggleFacebookSettings(syncEnabled, facebookSettingsPanel);
				toggleFacebookCommerceSettings(
					syncEnabled,
					facebookSettingsPanel,
				);

				if (syncEnabled) {
					removeProductIDFromUnSyncList(
						getSyncTargetProductID(simpleProductSyncModeSelect),
					);
				}

				simpleProductSyncModeSelect.prop(
					"original",
					simpleProductSyncModeSelect.val(),
				);

				if (
					shouldShowProductRemovedFromSyncConfirmModal(
						simpleProductSyncModeSelect,
					)
				) {
					showProductRemovedFromSyncConfirmModal(
						simpleProductSyncModeSelect,
					);
				}
			})
		...
		$productData.on(
			...
			function (event) {
				...
				setTimeout(function () {
					toggleFacebookSellOnInstagramSetting(
						isProductReadyForCommerce(),
						$("#facebook_options"),
					);
				}, 1);
			},
		...
		$(".woocommerce_variations").on(
			"change",
			".js-variable-fb-sync-toggle",
			function () {
				...
				toggleFacebookSettings(
					syncEnabled,
					$syncModeSelect.closest(".wc-metabox-content"),
				);
				toggleFacebookSellOnInstagramSetting(
					isProductReadyForCommerce(),
					$("#facebook_options"),
		...
		$productData.on("woocommerce_variations_loaded", function () {
			$productData
				.find(".js-variable-fb-sync-toggle")
				.each(function (index, element) {
					let $syncModeSelect = $(element);
					toggleFacebookSettings(
						$syncModeSelect.val() !== "sync_disabled",
						$syncModeSelect.closest(".wc-metabox-content"),
					);
					$syncModeSelect.prop("original", $syncModeSelect.val());
			...
			$(".variable_is_virtual").on("change", function () {
				const jsSyncModeToggle = $(this)
					.closest(".wc-metabox-content")
					.find(".js-variable-fb-sync-toggle");
				toggleSyncAndShowOption(
					!$(this).prop("checked"),
					jsSyncModeToggle,
		...
		$productData.on("change", ".js-fb-product-image-source", function () {
			let $container = $(this).closest(
				".woocommerce_options_panel, .wc-metabox-content",
			);
			let imageSource = $(this).val();

			$container
				.find(".product-image-source-field")
				.closest(".form-field")
				.hide();
			$container
				.find(`.show-if-product-image-source-${imageSource}`)
				.closest(".form-field")
				.show();
		});
		...
		$("#facebook_options").on(
			"click",
			"#product-not-ready-notice-open-modal",
			function (event) {
				event.preventDefault();
				...
				new $.WCBackboneModal.View({
					...
					string: {
						message:
							facebook_for_woocommerce_products_admin.product_not_ready_modal_message,
						buttons:
							facebook_for_woocommerce_products_admin.product_not_ready_modal_buttons,
					},
				});
			},
		);
