...
jQuery(document).ready(function ($) {
	...
	function wcFacebookProductSetInit() {
		...
		jQuery(document).ajaxSuccess(function (e, request, settings) {
			...
			if (
				obj.has("action") &&
				"add-tag" === obj.get("action") &&
				obj.has("taxonomy") &&
				"fb_product_set" === obj.get("taxonomy")
			) {
				jQuery(".select2.wc-facebook")
					.select2()
					.val(null)
	...
	$submitButton.on("click", function (e) {
		...
		if (
			...
			window.facebook_for_woocommerce_product_sets.excluded_category_ids
		) {
			excludedCategoryIDs =
				window.facebook_for_woocommerce_product_sets
		...
		if ($selectedCategories.length > 0 && excludedCategoryIDs.length > 0) {
			if (
				hasExcludedCategories($selectedCategories, excludedCategoryIDs)
			) {
				alert(
					facebook_for_woocommerce_product_sets.excluded_category_warning_message,
				);
			}
		}
	});
