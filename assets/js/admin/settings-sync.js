...
jQuery(document).ready(function ($) {
	...
	function getExcludedCategoriesAdded() {
		...
		if (
			window.facebook_for_woocommerce_settings_sync &&
			window.facebook_for_woocommerce_settings_sync.excluded_category_ids
		) {
			oldCategoryIDs =
				window.facebook_for_woocommerce_settings_sync
	...
	function getExcludedTagsAdded() {
		...
		if (
			window.facebook_for_woocommerce_settings_sync &&
			window.facebook_for_woocommerce_settings_sync.excluded_tag_ids
		) {
			oldTagIDs =
				window.facebook_for_woocommerce_settings_sync.excluded_tag_ids;
		}
	...
	$("input#wc_facebook_enable_product_sync")
	...
	$('input[name="save_product_sync_settings"]').on("click", function (e) {
		...
		if (categoriesAdded.length > 0 || tagsAdded.length > 0) {
			$.post(
				facebook_for_woocommerce_settings_sync.ajax_url,
				{
					action: "facebook_for_woocommerce_set_excluded_terms_prompt",
					security:
						facebook_for_woocommerce_settings_sync.set_excluded_terms_prompt_nonce,
				...
				function (response) {
					if (response && !response.success) {
						...
						$("#wc-backbone-modal-dialog .modal-close").trigger(
							"click",
						);
						...
						$(
							".facebook-for-woocommerce-confirm-settings-change",
						).on("click", function () {
							blockModal();
		...
		if (confirm(facebook_for_woocommerce_settings_sync.i18n.confirm_sync)) {
			...
			$.post(
				...
				function (response) {
					...
					if (!response.success) {
						let error =
							facebook_for_woocommerce_settings_sync.i18n
								.general_error;
				...
				clearSyncInProgress(
					facebook_for_woocommerce_settings_sync.i18n.general_error,
				);
			});
		}
	});
	...
	function setProductSyncStatus(count = null) {
		...
		$(
			'input#wc_facebook_enable_product_sync, input[name="save_product_sync_settings"]',
		)
			.css("pointer-events", "none")
			.css("opacity", "0.4");

		let message =
			facebook_for_woocommerce_settings_sync.i18n.sync_in_progress;
		...
		if (count) {
			if (count > 1) {
				message =
					message +
					facebook_for_woocommerce_settings_sync.i18n
						.sync_remaining_items_plural;
			} else {
				message =
					message +
					facebook_for_woocommerce_settings_sync.i18n
						.sync_remaining_items_singular;
			}
