	...
	window.WC_Facebook_Google_Product_Category_Fields = class WC_Facebook_Google_Product_Category_Fields {
		...
		constructor(categories, input_id) {
			...
			$('<div id="wc-facebook-google-product-category-fields"></div>')
				...
				.on(
					"change",
					"select.wc-facebook-google-product-category-select",
					(event) => {
			...
			var optionalSelectorID =
				this.globalsHolder().enhanced_attribute_optional_selector;
		...
		globalsHolder() {
			if (
				typeof facebook_for_woocommerce_product_categories !==
				"undefined"
			) {
				return facebook_for_woocommerce_product_categories;
			} else if (
				typeof facebook_for_woocommerce_settings_sync !== "undefined"
			) {
		...
		getPageType() {
			if (
				...
				if ($("input[name=tag_ID]").length === 0) {
					return this.globalsHolder()
						.enhanced_attribute_page_type_add_category;
				} else {
					return this.globalsHolder()
						.enhanced_attribute_page_type_edit_category;
				}
			} else {
				return this.globalsHolder()
					.enhanced_attribute_page_type_edit_product;
			}
		}
		...
		requestAttributesIfValid() {
			...
			// product sync page)
			var canShowEnhancedAttributesID =
			...
			if (this.isValid()) {
				...
				$.get(
					...
					function (response) {
						...
						$("#" + optionalSelectorID, $response).on(
							"change",
							function () {
								$(
									".wc-facebook-enhanced-catalog-attribute-optional-row",
		...
		onChange(element) {
			...
			if (element.hasClass("locked")) {
				element
					.closest(".wc-facebook-google-product-category-field")
					.nextAll()
					.remove();
			}
			...
			if (categoryId) {
				...
				categoryId = element
					.closest("#wc-facebook-google-product-category-fields")
		 ...
		 */
		isValid() {
			var selectsWithValueCount = $(
				".wc-facebook-google-product-category-select",
		...
		addSelect(options, selected) {
			...
			var $otherSelects = $container.find(
				".wc-facebook-google-product-category-select",
			);
			var $select = $(
				'<select class="wc-facebook-google-product-category-select"></select>',
			);
			...
			$container.append(
				$(
					'<div class="wc-facebook-google-product-category-field" style="margin-bottom: 16px">',
				).append($select),
			);
			...
			$select
				.attr(
					"data-placeholder",
					this.getSelectPlaceholder($otherSelects, options),
				)
		...
		getSelectPlaceholder($otherSelects, options) {
			if (0 === $otherSelects.length) {
				return facebook_for_woocommerce_google_product_category.i18n
					.top_level_dropdown_placeholder;
			}

			if (
				1 === $otherSelects.length &&
				0 === Object.keys(options).length
			) {
				return facebook_for_woocommerce_google_product_category.i18n
					.second_level_empty_dropdown_placeholder;
			}
		...
		getOptions(category_id) {
			...
			if (
				"undefined" === typeof this.categories[category_id]["options"]
			) {
				return [];
			}
		...
		getSelectedCategoryIds(categoryId) {
			...
			do {
				if ("undefined" !== typeof this.categories[categoryId]) {
					options.push([
						categoryId,
						this.categories[categoryId].parent,
					]);
